...
export const insertBatchToDatabase = async (data: any[], tableName: string) => {
  ...
  data.forEach(row => {
    ...
    const valueString = Object.keys(row).map(key => {
      let value = row[key];
      ...
      if (value === null || value === undefined || value === '') {
        console.log(`Valor inválido ou vazio na coluna '${key}' com valor: '${value}'`);
        return 'NULL';
  ...
  if (values.length > 0) {
    ...
    insertPromises.push(client.query(sql).catch((err) => {
  ...
  console.log(`Total de registros duplicados encontrados: ${Object.keys(duplicateIds).length}`);
